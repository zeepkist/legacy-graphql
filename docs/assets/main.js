(() => {
	...
	const _e = Object.getPrototypeOf;
		...
		(function () {
			...
			(t.Set = function (e) {
				...
				(t.Set.prototype.contains = function (e) {
					return Boolean(this.elements[e]);
				}),
			...
			(t.tokenizer = (e, n) => {
				...
				if (Array.isArray(e))
					return e.map(
						(y) =>
							new t.Token(t.utils.asString(y).toLowerCase(), t.utils.clone(n)),
					);
				for (
...
`,
			...
			(t.stemmer = (() => {
				...
				const k = /(at|bl|iz)$/;
				const _ = /([^aeiouylsz])\1$/;
				const B = new RegExp('^' + s + i + '[^aeiouwxy]$');
				const A = /^(.+?[^aeiou])y$/;
				const index =
				...
				const C = /^(.+?)e$/;
				const z = /ll$/;
				const W = new RegExp('^' + s + i + '[^aeiouwxy]$');
				const N = (c) => {
					let v;
					let P;
					let T;
			...
			(t.TokenSet = function () {
				...
				(t.TokenSet.prototype.toArray = function () {
					for (var e = [], n = [{ prefix: '', node: this }]; n.length > 0; ) {
						const r = n.pop();
						...
						for (
							...
							for (let d = s[u], y = 0; y < l; y++) {
								...
								if (p == d || d == '*') {
									...
									const g = r.qNode.edges[d];
									const L = b.final && g.final;
									let f = void 0;
									p in r.output.edges
				...
				(t.TokenSet.Builder.prototype.insert = function (e) {
					...
					this.minimize(r),
						this.uncheckedNodes.length === 0
							? (n = this.root)
							: (n = this.uncheckedNodes.at(-1).child);
				...
				(t.Index.prototype.query = function (e) {
					...
					for (var l = 0; l < n.clauses.length; l++) {
						...
						for (const b of d) {
							...
							const L = this.tokenSet.intersect(g).toArray();
							...
							for (let S = 0; S < L.length; S++)
								for (
									...
									if (
										(i[m].upsert(_, u.boost, (Qe, Ie) => Qe + Ie), !s[index])
									) {
										for (const q of A) {
			...
			(t.Builder = function () {
				...
				(t.Builder.prototype.add = function (e, n) {
					...
					for (let s = 0; s < i.length; s++) {
						...
						const l = a ? a(e) : e[o];
						const u = this.tokenizer(l, { fields: [o] });
						const d = this.pipeline.run(u);
						const y = new t.FieldRef(r, o);
						const p = Object.create(null);
						...
						for (const g of d) {
							...
							for (let m = 0; m < this.metadataWhitelist.length; m++) {
								const S = this.metadataWhitelist[m];
								const w = g.metadata[S];
					...
					if (n != null)
						switch (n.type) {
							...
							default: {
								...
								throw (
									(n.str.length > 0 && (r += " with value '" + n.str + "'"),
				...
				(t.QueryParser.parseField = (e) => {
					const n = e.consumeLexeme();
					if (n != null) {
						if (!e.query.allFields.includes(n.str)) {
							const r = e.query.allFields.map((o) => "'" + o + "'").join(', ');
							var i =
								"unrecognised field '" + n.str + "', possible fields: " + r;
				...
				(t.QueryParser.parseTerm = (e) => {
					...
					if (n != null) {
						(e.currentClause.term = n.str.toLowerCase()),
							n.str.includes('*') && (e.currentClause.usePipeline = !1);
	...
	const U = class {
		...
		createComponents(e) {
			for (const n of re) {
				for (const r of e.querySelectorAll(n.selector)) {
		...
		scrollToHash() {
			if (location.hash) {
				...
				const e = document.getElementById(location.hash.slice(1));
				if (!e) return;
		...
		listenForCodeCopies() {
			for (const e of document.querySelectorAll('pre > button')) {
				let n;
	...
	async function le(t, e) {
		if (!globalThis.searchData) return;
		const n = await fetch(globalThis.searchData);
		const r = new Blob([await n.arrayBuffer()])
			.stream()
			.pipeThrough(new DecompressionStream('gzip'));
		const i = await new Response(r).json();
		(t.data = i),
	...
	function he() {
		const t = document.querySelector('#tsd-search');
		if (!t) return;
		const e = { base: t.dataset.base + '/' };
		const n = document.querySelector('#tsd-search-script');
	...
	function Ve(t, e, n, r) {
		...
		for (let o = 0, a = Math.min(10, s.length); o < a; o++) {
			...
			const u = `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" class="tsd-kind-icon"><use href="#icon-${l.kind}"></use></svg>`;
			let d = ce(l.name, i);
	...
	function Ne(t, e) {
		let n = t.querySelector('.current');
		if (((n ||= t.querySelector('li:first-child')), n)) {
			const r = n.querySelector('a');
			r && (globalThis.location.href = r.href), e.blur();
		}
	}
	...
	function ce(t, e) {
		...
		while (o != -1)
			i.push(K(t.substring(s, o)), `<b>${K(t.substring(o, o + r.length))}</b>`),
				(s = o + r.length),
				(o = n.indexOf(r, s));
		return i.push(K(t.slice(Math.max(0, s)))), i.join('');
	}
	...
	document.addEventListener(fe, (t) => {
		if (ee && !D) {
			const e = F == 'touchstart' ? t.targetTouches[0] : t;
			const n = J.x - (e.pageX || 0);
			const r = J.y - (e.pageY || 0);
			D = Math.hypot(n, r) > 10;
		}
	});
	...
	const X = class extends I {
		...
		onDocumentPointerUp(e) {
			if (!D && this.active && e.target.closest('.col-sidebar')) {
				...
				if (n) {
					let r = globalThis.location.href;
					r.includes('#') && (r = r.slice(0, Math.max(0, r.indexOf('#')))),
						n.href.slice(0, Math.max(0, r.length)) == r &&
	...
	const Q = te;
	const ye = document.head.appendChild(document.createElement('style'));
...
`),
	...
	const Z = class extends I {
		constructor(e) {
			super(e),
				...
				(this.key = `tsd-accordion-${this.summary.dataset.key ?? this.summary.textContent.trim().replaceAll(/\s+/g, '-').toLowerCase()}`);
	...
	let Le;
	function be() {
		const t = document.querySelector('#tsd-nav-script');
		t && (t.addEventListener('load', xe), xe());
	}

	async function xe() {
		const t = document.querySelector('#tsd-nav-container');
		if (!t || !globalThis.navigationData) return;
		const n = await (await fetch(globalThis.navigationData)).arrayBuffer();
		const r = new Blob([n])
			.stream()
			.pipeThrough(new DecompressionStream('gzip'));
		const i = await new Response(r).json();
		...
		globalThis.app.createComponents(t),
			globalThis.app.showPage(),
			globalThis.app.ensureActivePageVisible();
	}
	...
	const Se = document.querySelector('#tsd-theme');
	Se && ge(Se);
	const je = new U();
	Object.defineProperty(globalThis, 'app', { value: je });
	he();
	be();
})();
